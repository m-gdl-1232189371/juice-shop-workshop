...
describe("/snippets/fixes/:key", () => {
  it("GET fixes for unknown challenge key throws error", () => {
    return frisby
      .get(URL + "/snippets/fixes/doesNotExistChallenge")
      .expect("status", 404)
  ...
  it("GET fixes for existing challenge key", () => {
    return frisby
      .get(URL + "/snippets/fixes/resetPasswordBenderChallenge")
      .expect("status", 200)
      .expect("jsonTypes", {
  ...
  it("POST wrong fix for existing challenge key gives negative verdict and explanation", () => {
    return frisby
      ...
      .expect("json", {
        verdict: false,
        explanation:
          "While not necessarily as trivial to research via a user's LinkedIn profile, the question is still easy to research or brute force when answered truthfully.",
      });
  });
  ...
  it("POST correct fix for existing challenge key gives positive verdict and explanation", async () => {
    ...
    await frisby
      ...
      .expect("json", {
        verdict: true,
        explanation:
          "When answered truthfully, all security questions are susceptible to online research (on Facebook, LinkedIn etc.) and often even brute force. If at all, they should not be used as the only factor for a security-relevant function.",
      })
      .promise();
