...
export const solve = function (challenge: any, isRestore = false) {
  ...
  challenge
    .save()
    .then(
      (solvedChallenge: { difficulty: number; key: string; name: string }) => {
        logger.info(
          `${isRestore ? colors.grey("Restored") : colors.green("Solved")} ${
            solvedChallenge.difficulty
          }-star ${colors.cyan(solvedChallenge.key)} (${solvedChallenge.name})`
        );
        ...
        if (!isRestore) {
          ...
          if (process.env.SOLUTIONS_WEBHOOK) {
            webhook
              .notify(solvedChallenge, cheatScore)
              .catch((error: unknown) => {
                logger.error(
                  "Webhook notification failed: " +
                    colors.red(utils.getErrorMessage(error))
                );
              });
          }
        }
...
export const sendNotification = function (
  challenge: { difficulty?: number; key: any; name: any; description?: any },
  isRestore: boolean
) {
  if (!notSolved(challenge)) {
    ...
    const wasPreviouslyShown =
      notifications.find(
        ({ key }: { key: string }) => key === challenge.key
      ) !== undefined;
...
export const sendCodingChallengeNotification = function (challenge: {
  key: string;
...
export const solveFindIt = async function (key: string, isRestore: boolean) {
  ...
  await ChallengeModel.update(
    ...
    { where: { key, codingChallengeStatus: { [Op.lt]: 2 } } }
  );
  logger.info(
    `${
      isRestore ? colors.grey("Restored") : colors.green("Solved")
    } 'Find It' phase of coding challenge ${colors.cyan(
      solvedChallenge.key
    )} (${solvedChallenge.name})`
  );
  if (!isRestore) {
...
export const solveFixIt = async function (key: string, isRestore: boolean) {
  ...
  logger.info(
    `${
      isRestore ? colors.grey("Restored") : colors.green("Solved")
    } 'Fix It' phase of coding challenge ${colors.cyan(solvedChallenge.key)} (${
      solvedChallenge.name
    })`
  );
  if (!isRestore) {
