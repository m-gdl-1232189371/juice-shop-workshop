...
export const isAuthorized = () => (expressJwt as any)(({ secret: publicKey }) as any)
export const denyAll = () => (expressJwt as any)({ secret: '' + Math.random() } as any)
export const authorize = (user = {}) => jwt.sign(user, privateKey, { expiresIn: '6h', algorithm: 'RS256' })
export const verify = (token: string) => token ? (jws.verify as ((token: string, secret: string) => boolean))(token, publicKey) : false
export const decode = (token: string) => { return (jws.decode(token) as any)?.payload }
